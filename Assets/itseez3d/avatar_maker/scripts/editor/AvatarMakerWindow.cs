...
namespace ItSeez3D.AvatarMaker.Editor
{
	public class AvatarMakerWindow : BaseWindow
	{
        ...
        private void ConnectEvent(byte[] obj)
        {
           // Debug.Log("On AvatarMakerWindow Reveice Message ");
           //string path = Application.streamingAssetsPath + "/timg.jpg";

           //byte[] bytes = File.ReadAllBytes(path);

           EditorRunner.instance.Run(GenerateAndDisplayHeadRoutine(obj, pipelineType));
		}
        ...
        private void OnEnable()
		{
            ...
            NetManager.Instance.ConnectEvent += ConnectEvent;
            Debug.Log("AvatarMakerWindow Init");
		...
		protected virtual IEnumerator GenerateAndDisplayHeadRoutine(byte[] photoBytes, PipelineType pipeline, Action<AsyncRequest> callback = null)
		{
			...
			Debug.Log("avatarCode is " + avatarCode);
            ...
            else
            {
				...
				NetManager.Instance.SendErrorMessage();
            }
		...
		/// <summary>
		/// 我自己插入的方法，主要是优化网格，输出数据 
		/// </summary>
        private void MyHeadHandle()
        {
            if (Selection.activeGameObject)
            {
                selectedAvatarInfo = Selection.activeGameObject.GetComponent<AvatarInfo>();

                selectedAvatarInfo.levelOfDetails = 8;//优化网格
                EditorRunner.instance.Run(UpdateAvatarMesh(selectedAvatarInfo,(() =>
                {
                    MyGeneraterMesh(selectedAvatarInfo.transform.Find("ItSeez3D Head").gameObject);
				})));
            }
            else
            {
        ...
        private void MyGeneraterMesh(GameObject headMesh)
        {
            if (headMesh == null)
            {
				Debug.LogError("拿不到头像");
                return;
            }

            SkinnedMeshRenderer skmr = headMesh.GetComponent<SkinnedMeshRenderer>();
			//GameObject go = new GameObject("myHead");
			//Mesh mesh = new Mesh();
			//mesh.vertices = skmr.sharedMesh.vertices;
   //         mesh.uv = skmr.sharedMesh.uv;
   //         mesh.triangles = skmr.sharedMesh.triangles;
   //         mesh.RecalculateTangents();
   //         mesh.RecalculateNormals();
   //         MeshFilter mf = go.AddComponent<MeshFilter>();
   //         mf.mesh = mesh;
   //         MeshRenderer mr = go.AddComponent<MeshRenderer>();
   //         mr.material.mainTexture = headMesh.GetComponent<Renderer>().material.mainTexture;


			Texture2D tex = (Texture2D)skmr.sharedMaterial.mainTexture;
            byte [] texByte = tex.EncodeToJPG();

			List<Vertices> vertices = new List<Vertices>();
            foreach (Vector3 vector3 in skmr.sharedMesh.vertices)
            {
				Vertices data = new Vertices();
                data.vertices_x = vector3.x;
                data.vertices_y = vector3.y;
                data.vertices_z = vector3.z;
                vertices.Add(data);
            }

			List<UVs> uVses = new List<UVs>();
            foreach (Vector2 vector2 in skmr.sharedMesh.uv)
            {
                UVs uv = new UVs();
                uv.UV_x = vector2.x;
                uv.UV_y = vector2.y;
            ...
            foreach (int triangle in skmr.sharedMesh.triangles)
            {
			...
			NetManager.Instance.SendMessageToSocket(bytes);
			...
			//File.WriteAllBytes(Application.streamingAssetsPath+"/test.byte",bytes);



			//         byte[] headBytes = File.ReadAllBytes(Application.streamingAssetsPath + "/test.byte");
			//         BinaryFormatter formatter = new BinaryFormatter();
			//         MemoryStream rems = new MemoryStream(headBytes);
			...
			//         meshNew.vertices = headInfo.GetVertices();
		...
		private IEnumerator UpdateAvatarMesh(AvatarInfo avatarInfo,Action action=null)
		{
